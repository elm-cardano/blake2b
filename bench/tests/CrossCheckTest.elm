            ...
            [ test "empty input" <|
                \_ ->
                    Blake2b.V2.hash512 emptyBytes
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 emptyBytes |> bytesToHex)
            , test "abc" <|
                \_ ->
                    Blake2b.V2.hash512 (hexToBytes "616263")
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 (hexToBytes "616263") |> bytesToHex)
            , test "128 bytes" <|
                \_ ->
                    Blake2b.V2.hash512 (sequentialBytes 128)
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 (sequentialBytes 128) |> bytesToHex)
            , test "129 bytes" <|
                \_ ->
                    Blake2b.V2.hash512 (sequentialBytes 129)
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 (sequentialBytes 129) |> bytesToHex)
            , test "255 bytes" <|
                \_ ->
                    Blake2b.V2.hash512 (sequentialBytes 255)
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 (sequentialBytes 255) |> bytesToHex)
            , test "1024 bytes" <|
                \_ ->
                    Blake2b.V2.hash512 (sequentialBytes 1024)
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash512 (sequentialBytes 1024) |> bytesToHex)
            ]
        , describe "keyed hash"
            [ test "64-byte keyed message" <|
                \_ ->
                    Blake2b.V2.hash { digestLength = 64, key = katKey, data = sequentialBytes 64 }
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash { digestLength = 64, key = katKey, data = sequentialBytes 64 } |> bytesToHex)
            , test "128-byte keyed message" <|
                \_ ->
                    Blake2b.V2.hash { digestLength = 64, key = katKey, data = sequentialBytes 128 }
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash { digestLength = 64, key = katKey, data = sequentialBytes 128 } |> bytesToHex)
            ]
        , describe "hash256"
            [ test "abc" <|
                \_ ->
                    Blake2b.V2.hash256 (hexToBytes "616263")
                        |> bytesToHex
                        |> Expect.equal (Blake2b.V1.hash256 (hexToBytes "616263") |> bytesToHex)
            ]
            ...
            [ test "V2 grand hash matches V1 grand hash" <|
                \_ ->
                    ...
                    grandHash { hash = Blake2b.V2.hash }
                        |> Expect.equal (grandHash { hash = Blake2b.V1.hash })
            ]
        ]
